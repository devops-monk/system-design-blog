...
Standard REST, and the interesting part is who is allowed to call what:

| Endpoint | Who |
|---|---|
| `GET /v1/hotels/{id}` · `GET /v1/hotels/{id}/rooms/{id}` | Anyone |
| `POST` · `PUT` · `DELETE` on `/v1/hotels` and `/v1/hotels/{id}/rooms` | **Ops only** |
| `GET /v1/reservations` · `GET /v1/reservations/{id}` | The signed-in user |
| `POST /v1/reservations` | The signed-in user |
| `DELETE /v1/reservations/{id}` | The signed-in user |

**The management endpoints are not internet-facing at all** — they sit behind the internal VPN in the architecture below. Hotel staff changing room inventory and a guest booking a room are different systems that happen to share a database.

A reservation request is unremarkable except for one field:
...
**`reservationID` is supplied by the client, not generated by the server.** That looks wrong the first time you see it. It is the single most important detail in this chapter, and the double-booking section is where it earns its place.

Search endpoints are deliberately out of scope. Real systems need filtering by city, date range, price, amenities and availability — but none of it is technically hard, and it distracts from the part that is.
